...
def isSorted(x: list, orient: None):
    ...
    if orient == True or orient == None:
        sorted = True
        highestn = -9999999999999999999999
        for n in x:
            if n >= highestn:
                highestn = n
            elif n < highestn:
...
def dec2hex(x: int):
    """Converts decimal to hexadecimal. Input x must be an integer."""
    outputNum = ""
    table = {
        10: "A",
        11: "B",
    ...
    while x >= 1:
        rem = x % 16
        if rem < 10:
            outputNum += str(int(rem))
        else:
            outputNum += table.get(rem, 0)
        if x < int(x / 16):
           x = int(x / 16)
        elif x > int(x / 16):
           x = int(x / 16)
...
def bin2hex(x: str):
    """Converts binary to hexadecimal. Input x must be a string."""
    return dec2hex(bin2dec(x))

def oct2bin(x: str):
    """Converts octal to binary. Input x must be a string."""
    table = {
        '0': '000',
        '1': '001',
        '2': '010',
...
def oct2dec(x: str):
    """Converts octal to decimal. Input x must be a string."""
    initX = len(x)-1
    res = 0
    for char in x:
...
def oct2hex(x: str):
    """Converts octal to hexadecimal. Input x must be a string."""
    result1 = ''
    for char in x:
        result1 += oct2bin(char)
    return bin2hex(result1)